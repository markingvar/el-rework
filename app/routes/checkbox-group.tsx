...
import { checkboxGroupFormBlueprint } from "~/forms/checkbox-group-form";
import {
  formActionFunction,
  formLoaderFunction,
...
export let loader: LoaderFunction = async ({ request }) => {
  const { formStructure, context, commitSession, session } =
    await formLoaderFunction({
      request,
      formBlueprint: checkboxGroupFormBlueprint,
  ...
  return (
    <>
      <div className="content-wrapper form-content-wrapper">
        <ElectricLadylandForm context={context} formStructure={formStructure} />
      </div>
    </>
  );
}
